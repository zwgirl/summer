 ...
 */
final public class SsClasspathContainer implements IClasspathContainer {

	private static final String XTEXT_XBASE_LIB_BUNDLE_ID = "org.summer.dsl.xbase.lib";
	private static final String XTEND_LIB_BUNDLE_ID = "org.summer.ss.lib";
	...
	private static final Logger LOG = Logger.getLogger(SsClasspathContainer.class);

	private final IPath containerPath;
	private IClasspathEntry[] classPathEnries;

	SsClasspathContainer(IPath containerPath) {
	...
	public IClasspathEntry[] getClasspathEntries() {
		if (classPathEnries == null) {
			...
			for (String bundleId : SsClasspathContainer.BUNDLE_IDS_TO_INCLUDE) {
	...
	private void addEntry(final List<IClasspathEntry> cpEntries, final String bundleId) {
		...
		if (bundle != null) {
			...
			if (SsClasspathContainer.XTEXT_XBASE_LIB_BUNDLE_ID.equals(bundleId)
					|| SsClasspathContainer.XTEND_LIB_BUNDLE_ID.equals(bundleId)) {
	...
	private String calculateJavadocURL() {
		Version myVersion = SsActivator.getInstance().getBundle().getVersion();

...
public class JvmModelGenerator implements IGenerator {
  ...
  private IQualifiedNameConverter qualifiedNameConverter;
	...
	private TreeAppendable createAppendable(EObject context, ImportManager importManager, GeneratorConfig config) {
		TreeAppendable appendable = new TreeAppendable(importManager, converter, locationProvider, jvmModelAssociations, context, "  ", "\n");
		JvmGenericType type = containerType(context);
		if( type != null) {
			appendable.declareVariable(type, "this");
			JvmTypeReference superType = type.getExtendedClass();
			if (superType != null)
	...
	private JvmGenericType containerType(EObject context) {
		if(context == null) 
			return null;
		if(context instanceof JvmGenericType)
			return (JvmGenericType) context;
		else return containerType(context.eContainer());
	}
...
//		}
	...
	public void internalDoGenerate(EObject obj, TreeAppendable b) {
		if(obj instanceof JvmEnumerationType){
			...
			compiler.toJavaStatement((XVariableDeclarationList)obj, b, false);
		} else if(obj instanceof JvmInterfaceType){
			_generateInterfaceType((JvmInterfaceType) obj, b);
		} else if(obj instanceof XFunctionDeclaration){
			compiler.toJavaStatement((XFunctionDeclaration)obj, b, false);
		} else{
			compiler.toJavaStatement((XStatment)obj, b, false);
		}
	}
	...
	protected void _generateInterfaceType(JvmInterfaceType interfaceType,
			TreeAppendable b) {
		b.newLine();
		b.append("var ").append(interfaceType.getSimpleName()).append(" = ")
				.append("{");
		b.increaseIndentation();
		b.newLine();
		Iterable<JvmField> fields = interfaceType.getDeclaredFields();

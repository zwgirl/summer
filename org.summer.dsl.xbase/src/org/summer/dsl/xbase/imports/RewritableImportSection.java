...
//		boolean needNewline = appendSubsection(builder, filter(allDeclarations, new Predicate<XImportDeclaration1>() {
//			public boolean apply(XImportDeclaration1 input) {
//				return !input.isStatic();
//			}
//		}), false);
//		needNewline = appendSubsection(builder, filter(allDeclarations, new Predicate<XImportDeclaration1>() {
//			public boolean apply(XImportDeclaration1 input) {
//				return input.isStatic() && !input.isExtension();
//			}
//		}), needNewline);
//		appendSubsection(builder, filter(allDeclarations, new Predicate<XImportDeclaration1>() {
//			public boolean apply(XImportDeclaration1 input) {
//				return input.isStatic() && input.isExtension();
//			}
//		}), needNewline);
	...
	protected boolean appendSubsection(StringBuilder builder, Iterable<XImportDeclaration> subSection, boolean needsNewline) {
		if (!isEmpty(subSection)) {
			if(needsNewline)
				builder.append(lineSeparator);
//			for (XImportDeclaration1 declaration: isSort() ? sort(subSection) : subSection) {
...
//	protected List<XImportDeclaration> sort(Iterable<XImportDeclaration1> declarations) {
//		List<XImportDeclaration1> sortMe = newArrayList(filter(declarations, new Predicate<XImportDeclaration1>() {
//			public boolean apply(XImportDeclaration1 in) {
//				return !isEmpty(in.getImportedTypeName());
//			}
//		}));
//		Collections.sort(sortMe, new Comparator<XImportDeclaration1>() {
//			public int compare(XImportDeclaration1 o1, XImportDeclaration1 o2) {

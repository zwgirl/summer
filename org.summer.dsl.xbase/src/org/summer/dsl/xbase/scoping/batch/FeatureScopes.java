...
public class FeatureScopes implements IFeatureNames {
	...
	public IScope createSimpleFeatureCallScope(EObject context, EReference reference, IFeatureScopeSession session, IResolvedTypes resolvedTypes) {
		...
		//cym add
		IScope rootTypeScope = new RootScope(context.eResource(), implicitReceivers);
		//cym add end
		
//		IScope constructors = createConstructorDelegates(context, implicitReceivers, session, resolvedTypes);  //cym  comment
		IScope constructors = createConstructorDelegates(context, rootTypeScope, session, resolvedTypes);
